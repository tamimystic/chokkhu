...
class PersistenceLandscape:
    ...
    def __init__(
        ...
        self.n_landscapes = int(
            num_landscapes if num_landscapes is not None else n_landscapes
        )
    ...
    def transform_single(
        ...
        t_start = (
            ...
            if t_min is not None
                ...
                else float(np.min(valid_diag[:, 0]))
            )
        )
        t_end = (
            t_max
            if t_max is not None
                ...
                else float(np.max(valid_diag[:, 1]))
            )
        )
        if t_end <= t_start:
            t_end = t_start + 1.0
        ...
        landscapes: np.ndarray = np.zeros(
            (self.n_landscapes, self.n_bins), dtype=np.float64
        )
    ...
    def transform(
        ...
        if isinstance(diagrams, list):
            results = [
                self.transform_single(d, t_min=t_min, t_max=t_max).ravel()
                for d in diagrams
            ]
            return np.array(results, dtype=np.float64)

        if hasattr(diagrams, "pairs"):
            return self.transform_single(
                diagrams.pairs, t_min=t_min, t_max=t_max
            ).ravel()
        ...
        if arr.ndim <= 2:
            ...
            results = [
                self.transform_single(d, t_min=t_min, t_max=t_max).ravel() for d in arr
            ]
            return np.array(results, dtype=np.float64)
...
class PersistenceImage:
    ...
    def transform_single(self, diagram: np.ndarray) -> np.ndarray:
        ...
        b_min, b_max = (
            self.birth_range
            if self.birth_range
            else (float(np.min(births)), float(np.max(births)))
        )
        p_min, p_max = (
            self.pers_range if self.pers_range else (0.0, float(np.max(pers)))
        )
